...
const ModalEntry: React.FC<ModalEntryProps> = ({ show, onClose, setNewForm }) => {
    ...
    return (
        <div
            className={`modalEntry-bg ${show ? "block" : "hidden"}`}
            onClick={handleBackgroundClick} // Добавляем обработчик клика
        >
            <dialog ref={dialogRef} className="dialogEntry">
                <form onSubmit={handleFormSubmit} method="dialog">
                    <div
                        className={`modalEntry-frame ${isDarkMode ? 'dark' : 'light'}`}
                        onClick={(e) => e.stopPropagation()} // Останавливаем всплытие клика на модалке
                    >
                        {/* Заголовок Модального окна */}
                        <div className="modalEntry-header">
                            <h1 className="modalEntry-title">Войти</h1>
                        </div>
                        ...
                        />
                        <Link href="/forgot-password" className="linkPassword" onClick={onClose}>
                            <p>Забыли пароль?</p>
                        </Link>
